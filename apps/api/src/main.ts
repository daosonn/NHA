 ...
 * the API. Metro takes the next free port when 8081 is busy, so development
 * matches any localhost/127.0.0.1 port instead of naming fixed ones.
 *
...
function corsOrigin(): string[] | RegExp | null {
  const configured = process.env.CORS_ORIGINS?.trim();
  if (configured !== undefined && configured !== '') {
    return configured.split(',').map((origin) => origin.trim());
  }

  return process.env.NODE_ENV === 'production' ? null : DEV_ORIGIN_PATTERN;
}
