...
import { CreateVideoJobDto, ShareVideoDto, StoryboardRequestDto } from './dto/video.dto';
...
export class VideoController {
  ...
  @Get('video-music')
  @ApiOperation({ summary: 'Screen 29 — music library grouped by mood (built-in, no copyright issues)' })
  ...
  async musicFile(@Param('trackId') trackId: string, @Res() res: Response): Promise<void> {
    const { path: p, size } = await this.videoService.musicFileFor(trackId);
    res.status(200).set({ 'content-type': 'audio/mp4', 'content-length': String(size), 'cache-control': 'public, max-age=86400' });
    createReadStream(p).pipe(res);
  }

  @Post('families/:familyId/video-jobs/storyboard')
  @ApiOperation({ summary: 'Screen 27→31 — AI storyboard to review/edit BEFORE creating the job (1 AI call, mockable)' })
  storyboard(
  ...
  @ApiOperation({ summary: 'Create a video job — mode "ai" (edited plan) or "quick" (stitch photos in my order, 0 AI)' })
  create(
    @CurrentUser() user: AuthUser,
    @Param('familyId', ParseUUIDPipe) familyId: string,
  ...
  @ApiOperation({ summary: 'Screen 32 — start async render (poll GET /video-jobs/:id for progress/stage)' })
  render(@CurrentUser() user: AuthUser, @Param('jobId', ParseUUIDPipe) jobId: string): Promise<{ ok: boolean }> {
  ...
  get(@CurrentUser() user: AuthUser, @Param('jobId', ParseUUIDPipe) jobId: string): Promise<VideoJobView> {
    return this.videoService.get(user.userId, jobId);
  }

  @Get('video-jobs/:jobId/file')
  @ApiOperation({ summary: 'Stream the rendered mp4 (supports HTTP Range for seeking)' })
  async file(
    @CurrentUser() user: AuthUser,
    ...
    if (m && (m[1] || m[2])) {
      ...
      res.status(200).set({ 'content-type': 'video/mp4', 'content-length': String(size), 'accept-ranges': 'bytes' });
      this.videoService.stream(key).pipe(res);
    }
  }

  @Post('video-jobs/:jobId/share')
  @ApiOperation({ summary: 'Screen 33 — share to the family timeline as a post with the video attached' })
  share(
    @CurrentUser() user: AuthUser,
    @Param('jobId', ParseUUIDPipe) jobId: string,
