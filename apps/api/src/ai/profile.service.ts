...
export class ProfileService {
  ...
  constructor(
    ...
    this.rollupEvery = Math.max(
      1,
      Number(config.get('ROLLUP_EVERY_N_POSTS') ?? 1) || 1,
    );
  }
  ...
  async analyzePost(
    postId: string,
    locale = 'en',
  ): Promise<{ signals: number; rollup: RollupOutcome | null }> {
    const post = await this.prisma.post.findUnique({
      ...
      select: {
        ...
        memberTags: {
          select: { memberId: true, member: { select: { displayName: true } } },
        },
    ...
    for (const sig of result.signals) {
      ...
      await this.prisma.interestSignal.create({
        data: {
          ...
          confidence: Math.min(
            sig.confidence,
            CONFIDENCE_CAP[sourceType] ?? 0.85,
          ),
          basis: sig.basis,
          observedAt,
        },
    ...
    const rollup =
      written > 0 ? await this.bumpCounter(authorMember.id, locale) : null;
    this.logger.log(
      `analyze ${postId}: ${written} signal(s)${rollup?.ran ? `, rollup → v${rollup.newVersion}` : ''}`,
    );
  ...
  analyzePostInBackground(postId: string, locale = 'en'): void {
    const attempt = (n: number): void => {
      void this.analyzePost(postId, locale).catch((error) => {
        ...
        if (n === 1) {
          this.logger.warn(
            `analyze ${postId} lỗi (thử lại sau 8s): ${message}`,
          );
          setTimeout(() => attempt(2), 8_000);
          return;
        }
        this.logger.error(
          `analyze ${postId} thất bại lần 2, bỏ qua: ${message}`,
        );
      });
    };
    attempt(1);
  ...
  async hasUnprocessed(memberId: string): Promise<boolean> {
    return (
      (await this.prisma.interestSignal.count({
  ...
  async rollupMember(memberId: string, locale = 'en'): Promise<RollupOutcome> {
    ...
    if (signals.length === 0)
      return { ran: false, oldVersion, newVersion: oldVersion, signalsUsed: 0 };
    ...
    await this.prisma.$transaction([
      this.prisma.memberProfile.create({
        data: {
          memberId,
          version: newVersion,
          // Prisma nhận Json qua kiểu riêng của nó; hồ sơ là object thuần nên cast
          // là bước bắt buộc, không phải assertion dư (eslint --fix từng gỡ mất).
  ...
  async resolveEvidence(
    ...
    for (const ref of [...new Set(refs)].slice(0, 12)) {
      ...
      if (ref.startsWith('sig_') || ref.startsWith('post_')) {
        ...
        const postId = ref.startsWith('post_')
          ? ref.slice(5)
        ...
        const post = postId
          ...
          : null;
        const photo =
          post?.media.find((m) => m.mimeType.startsWith('image/')) ??
          post?.media[0] ??
          null;
        out.push({
          ref,
          ...
          created_at:
            (post?.createdAt ?? signal?.observedAt)?.toISOString() ?? null,
  ...
  async profileFor(userId: string, familyId: string, memberId: string) {
    ...
    const [latest, signalCount, pending] = await Promise.all([
      ...
      this.prisma.interestSignal.count({
        where: { memberId, processed: false, revoked: false },
      }),
    ]);
    return {
  ...
  private async relationSentences(
    familyId: string,
    memberId: string,
  ): Promise<string[]> {
    ...
    return edges.map((e) => {
      ...
      if (e.type === 'OTHER')
        return `related to ${other}${e.label ? ` (${e.label})` : ''}`;
      const pair = word[e.type];
      return pair
        ? `${iAmFrom ? pair[0] : pair[1]} of ${other}`
        : `related to ${other}`;
    });
  }
}
