...
| Post detail             | **Wired** — comments and reactions (`app/post/[id].tsx`), including editing and deleting your own comments (2026-08-21). Hiding or removing somebody else's is not offered: the post's author has no moderation power on the server. The separate `app/moments.tsx` was deleted when the feed moved into Home.                                    |
